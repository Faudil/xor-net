...
Current realistic single-stream: **~59 tok/s** (Int4 LM head + optimized
sampler, 8 threads). Aggregate 80+ is reachable via concurrent serving.
...
thrash + pool contention. Measured *decode (blocks)* tok/s vs rayon threads
(excludes LM head + sampling; this is the 13.86 ms/tok block figure below):

| threads | 1 | 2 | 3 | 4 | 5 | 6 | 8 | 12 |
|--------:|--:|--:|--:|--:|--:|--:|--:|--:|
| tok/s   | 22| 40| 57| 65| 63| 61| 65| 60|

Sweet spot **4–8 threads (~65 tok/s blocks)**; 12 threads *regresses* to 60.
`init_threads(0)` now auto-caps at 8 (see `src/lib.rs`).

**Full single-stream pipeline (HF `bitnet-b1.58-2B-4T`, 8 threads, measured):**

| config | end-to-end | forward | blocks | LM head | sample |
|--------|----------:|--------:|-------:|--------:|-------:|
| Int8 head + full-sort sampler (old) | 46.6 | 51.9 | 13.85 | 5.43 | 2.13 |
| Int4 head + full-sort sampler | ~52 | ~64 | 13.86 | 2.82 | 2.13 |
| **Int4 head + partial-select sampler (current)** | **58.8** | 59.8 | 13.86 | 2.82 | 0.28 |

So the 65 in the thread sweep above is the *blocks-only* decode rate; the
realistic single-stream ceiling out of the box is **~59 tok/s** (Int4 LM head
+ optimized sampler). The LM head is the remaining per-token cost.
...
So:
- **Single-stream ceiling ≈ 59 tok/s** (HF 2B, 8 threads, Int4 LM head +
  optimized sampler). Blocks decode alone (13.86 ms/tok) is 72 tok/s; the
  LM head is the remaining ~2.8 ms/tok cost and is **compute-bound** (it
  does not shrink with ternary  both Int4 and ternary measure ~2.8 ms/tok,
  since the 128k×2560 GEMV is MAC-bound, not weight-bandwidth-bound).
  **80 tok/s single-stream is NOT reachable losslessly** on this hardware
  without a faster LM-head kernel.
...
is not reachable** losslessly on this 6C/12T box (ceiling ~59 with the
current LM-head kernel). **80 tok/s aggregate IS reachable** by serving 6
concurrent single-threaded streams (~110 tok/s). The earlier XorSparse /
kernel / prefetch work, plus the Int4-LM-head + partial-select-sampler fixes,
confirmed weight/precision tricks alone don't move the needle past ~59.
...
- **Single-stream**: blocks decode is already ~72 tok/s; the bottleneck is
  now the **LM-head GEMV** (~2.8 ms/tok, compute-bound). The only lever left
  is a faster LM-head kernel (wider/unpacked int4 path, or overlapping the
  head compute with the last block)  a larger refactor. The 22 tok/s-per-core
  decode figure can't be lifted by precision tricks (Int4 and ternary LM head
  are both ~2.8 ms/tok). The sampler was already fixed: it no longer
  allocates a 128k `(usize,f32)` vector or full-sorts the vocab every token
  (`src/sampler.rs` now uses a reused index buffer + `select_nth_unstable`
  partial selection), dropping sample from 2.13 → 0.28 ms/tok.
