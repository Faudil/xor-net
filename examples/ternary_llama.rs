...
use candle_nn::{RmsNorm, Linear};
use xor_net::bit1_58::layers::TernaryLinear;
use xor_net::bit1_58::quantization::TernaryPackType;

pub fn linear(in_dim: usize, out_dim: usize, vb: candle_nn::VarBuilder) -> Result<TernaryLinear> {
    let tensor = vb.get((out_dim, in_dim), "weight")?;
    let weights_f32 = tensor.to_dtype(DType::F32)?.flatten_all()?.to_vec1::<f32>()?;
    let pack_type = TernaryPackType::Pack4;
    TernaryLinear::new(in_dim, out_dim, &weights_f32, pack_type)
...
struct CausalSelfAttention {
    ...
    head_dim: usize,
    use_flash_attn: bool,
    inner_attn_ln: candle_nn::RmsNorm,
...
impl CausalSelfAttention {
    ...
    fn forward(
        ...
        let y = self.inner_attn_ln.forward(&y)?;
    ...
    fn load(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        Ok(Self {
            ...
            inner_attn_ln: candle_nn::rms_norm(cfg.hidden_size, cfg.rms_norm_eps, vb.pp("inner_attn_ln"))?,
...
struct Mlp {
    ...
    ffn_layernorm: candle_nn::RmsNorm,
}

impl Mlp {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        let x = (candle_nn::ops::silu(&self.c_fc1.forward(x)?)? * self.c_fc2.forward(x)?)?;
        let x = self.ffn_layernorm.forward(&x)?;
    ...
    fn load(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        let ffn_layernorm = candle_nn::rms_norm(i_size, cfg.rms_norm_eps, vb.pp("ffn_layernorm"))?;
        Ok(Self {
            c_fc1,
            c_fc2,
            c_proj,
